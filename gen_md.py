...
for a in os.walk(sys.path[0]):
    for b in a[2]:
        if(b[-3:]in['txt','TXT'])and(a[0].find('档案图')==-1)and(a[0].find('马克思恩格斯全集1~50')==-1)and(a[0].find('列宁全集1~60')==-1)and(a[0].find('新编斯大林全集及档案附卷（简体横排本，诸夏怀斯社编）')==-1)and(a[0].find('TN 无线电电子学、电讯技术')==-1)and(a[0].find('TP 自动化技术、计算技术')==-1)and(a[0].find('TQ 化学工业')==-1):
            l.append([b,a[0]])
l.sort()
for b in l:
